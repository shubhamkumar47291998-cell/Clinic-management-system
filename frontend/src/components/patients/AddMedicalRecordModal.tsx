...
export const AddMedicalRecordModal: React.FC<AddMedicalRecordModalProps> = ({
  ...
  const [appointments, setAppointments] = useState<{ id: string; slot_start: string }[]>([]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Also sync to prescriptions table
      if (prescriptions.length > 0) {
        ...
        await supabase.from('prescriptions').insert({
          ...
          notes: `Consultation record notes: ${diagnosis.trim()}`
        });
      }
    ...
    <div className="modal-overlay" style={modalOverlayStyle}>
      <div className="modal-content" style={modalContentStyle}>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            >
              ...
              {appointments.map((ap) => (
                <option key={ap.id} value={ap.id}>
                  {new Date(ap.slot_start).toLocaleString()}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="form-group">
            <label>Digital Signature (Write Signature / Full Name) *</label>
            <input
              type="text"
              placeholder="e.g. Dr. Anisha Natekar"
              value={doctorSignature}
              onChange={(e) => setDoctorSignature(e.target.value)}
              style={{ width: '100%', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', backgroundColor: 'var(--bg-card)', color: 'var(--text-primary)' }}
              required
            />
          </div>
